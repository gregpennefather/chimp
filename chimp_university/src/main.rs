...
fn main() {
    ...
    // for rank in 4..7 {
    //     for file in 2..6 {
    //         let index = get_index_from_file_and_rank(file, rank);
    //         draw = draw.flip(index)
    //     }
    // }
    // println!("{draw}:\n{}", draw.to_board_format());

    println!("xe5 xe5: {}", Position::from_fen("rn1qk2r/pp5p/1b1bP1p1/2p2p2/1QP2P2/8/7P/RNB1K2R w KQkq - 1 9".into()).eval);
