...
pub fn is_legal_rook_move(m: Move, board: BoardRep) -> bool {
    if MOVE_DATA.is_slide_legal(m.from(), m.to()).0 {
        println!("{}", board.occupancy.to_board_format());
        println!("{}", MOVE_DATA.get_slide_inbetween(m.from(), m.to()).to_board_format());
...
#[cfg(test)]
mod test {
    use crate::{board::board_rep::BoardRep, shared::{board_utils::index_from_coords, piece_type::PieceType}, r#move::Move, move_generation::sliding::rook::is_legal_rook_move};
