...
fn perft(desired_depth: usize) {
    ...
    while depth < desired_depth {
        ...
        for path in paths {
            ...
            for board_state in path.1.iter() {
                ...
                for m in moves {
                    ...
                    move_node_count.entry(path.0).and_modify(|v| *v += 1);
                    node_count += 1;
                }
            }
        ...
        print_test_result(
            format!("Perft {}", depth + 1),
            format!("Nodes {}/{} - ({:?})", node_count, EXPECTED_NODE_COUNT[depth], duration).into(),
            node_count == EXPECTED_NODE_COUNT[depth],
        );
        ...
        if node_count != EXPECTED_NODE_COUNT[depth] {
            ...
            for &key in move_node_count.keys() {
                keys.push((get_move_uci(key), key))
            }
            keys.sort();
            for (name, key) in keys {
                println!("{}: {}", name, move_node_count.get(&key).unwrap())
            }
        }
        depth += 1;
    }
